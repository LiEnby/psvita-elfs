...
//		printf("[SEND][TOUCH START] id=" + ev.changedTouches[0].identifier + " x=" + ev.changedTouches[0].pageX + " y=" + ev.changedTouches[0].pageY);						
	...
	function PosCompare(event,touch,range)
	{
		if(touch.x - range > event.pageX || touch.x + range < event.pageX)
			return false;
		
		if(touch.y - range > event.pageY || touch.y + range < event.pageY)
			return false;
	...
	mapdoc.addEventListener('touchmove', function(ev)
	{
		...
		if(GoogleDragStart)
		{
			if(!DragPos)
			{
				DragPos = TouchEventDataToTouchPosData(ev.touches[0]);
	//			printf("[SEND][DRAG START] id=" + ev.changedTouches[0].identifier + " x=" + ev.changedTouches[0].pageX + " y=" + ev.changedTouches[0].pageY);						
				var cppContext = new CppContextHelper();
				cppContext.functionId = MAP_JSCALL_CALLBACK_EVENT;
				cppContext.eventType  = MapJSEventDragBegin;
				cppContext.screenX = DragPos.x;
				cppContext.screenY = DragPos.y;
				cppContext.AddParam(DragPos.id);
				cppContext.Serialize();
	...
	mapdoc.addEventListener('touchend', function(ev)
	{
		if(ev.touches.length == 0)
		{
			...
			{
				DragPos = null;
//				printf("[SEND][DRAG END] id=" + ev.changedTouches[0].identifier + " x=" + ev.changedTouches[0].pageX + " y=" + ev.changedTouches[0].pageY);						
				var cppContext = new CppContextHelper();
				cppContext.functionId = MAP_JSCALL_CALLBACK_EVENT;
				cppContext.eventType  = MapJSEventDragEnd;
				cppContext.screenX = ev.changedTouches[0].pageX;
				cppContext.screenY = ev.changedTouches[0].pageY;
				cppContext.AddParam(ev.changedTouches[0].identifier);
				cppContext.Serialize();
			}
			GoogleDragStart = false;
		}
